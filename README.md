...
That is it. Logo Yoink does not need a database, browser, account, or API key for its default extraction path.
...
Static discovery runs immediately. No setup step or environment file is required.
...
`npm run setup` installs the Node dependencies and Chromium. On a fresh Linux machine, use `npm run setup:linux` so Playwright also installs Chromium's system libraries. If you only want the default static library API, use `npm run setup:static`; it skips the browser download.
...
The CLI does not use a scraper by default. Add `--browser`, `--jina`, or `--all-fallbacks` explicitly. Jina also requires `JINA_API_KEY`.
...
To recover logos from JavaScript-rendered pages, opt into the local browser scraper:

```js
const result = await yoink('stripe.com', { scrapers: ['browser'] });
```
...
| Local Playwright browser | **Off** | **On** | Add `"browser"` to `scrapers`; send `[]` to the HTTP API to disable it |
...
`sitemap: true` enables a missing-wide-only experimental recovery pass in the JavaScript API. It reads robots-declared sitemaps on the exact registrable domain, fetches at most one official brand/press/logo or shallow corporate/about page, and admits only wide-role candidates under separate request, byte, redirect, and wall-clock ceilings. Low-intent corporate/about pages require exact company-plus-logo evidence on the candidate itself. It is off by default because the frozen major-brand development run recovered two reviewed-correct wordmarks, while a wider 102-miss development cohort and two validation cohorts safely abstained without confirming broader effectiveness.
