...
async function syntheticRun() {
  ...
  await writeFile(join(run, 'manifest.json'), JSON.stringify({ version: 'visual-benchmark-v1', run_id: 'synthetic' }));
  ...
  await jsonl(join(run, 'mappings.jsonl'), [{ entity_id: 'acme', visual_instance_id: 'header-1', candidate_id: 'candidate-raster', mapping_type: 'exact' }]);
...
test('parses packet options and loads versioned capture records', async () => {
  assert.deepEqual(parseArgs(['--run', 'runs/x', '--output=out', '--resume']), { run: 'runs/x', output: 'out', resume: true });
...
test('generates safe exhaustive review packet with empty annotation defaults', async () => {
  ...
  assert.match(entity, /data-field="best-role"/);
  assert.match(entity, /data-field="missing-cause"/);
  ...
  assert.match(entity, /label_id: labelId/);
  assert.match(entity, /label_kind: labelKind/);
  assert.match(entity, /reviewer_id: reviewer/);
  assert.match(entity, /prompt_version: 'visual-review-packet-v2'/);
