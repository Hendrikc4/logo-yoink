...
function fakeBrowser({ rejectClips = false } = {}) {
  const calls = [];
  const page = {
    on() {},
    async route() {},
    ...
    async evaluate(fn, argument) {
      if (fn.name === 'inspectPage') return [{ kind: 'img', source: 'browser-img', url: 'https://cdn.example.com/logo.svg', box: { x: 10, y: 12, width: 120, height: 24 }, region: 'header', homeLinked: true, alt: 'Example logo' }];
