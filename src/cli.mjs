...
const browser = allFallbacks || args.includes('--browser');
...
if (!website || downloadIndex >= 0 && !downloadDirectory ||
  ...
  console.error('Usage: logo-yoink <website> [--browser] [--jina] [--all-fallbacks] [--theme any|light|dark] [--background any|transparent|opaque] [--role icon|logo] [--download <directory>] [--no-wikimedia-fallback] [--bimi]');
  process.exit(1);
}
