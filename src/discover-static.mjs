...
export function parseHomepage(html, base, { companyName = '' } = {}) {
  ...
  const candidates = [], manifests = [], brandPages = [];
  let order = 0;
  ...
  for (const node of nodes) {
    ...
    if (node.name === 'noscript') {
      const nested = parseHomepage(DomUtils.textContent(node), documentBase, { companyName });
    ...
    if (node.name === 'a') {
      ...
      const label = `${DomUtils.textContent(node)} ${a.href ?? ''}`;
      if (href && /(?:^|[\s/_-])(brand|press|media|newsroom|about|company)(?:$|[\s/?#_-])/i.test(label)) {
  ...
  return { candidates, manifests: [...new Set(manifests)], brandPages: [...new Set(brandPages)] };
}

export const internals = { context, evidence, firstSrcset, isSelfContainedSvg };
