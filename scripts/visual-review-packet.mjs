...
const ROLES = ['icon', 'wide', 'favicon', 'stacked', 'other'];
const EMPTY = '';
...
const REVIEW_VERSION = 'visual-review-packet-v2';
...
export function stableReviewId(value) {
  let hash = 2166136261;
...
export function targetKeyFor({ labelKind = '', entityId = '', visualInstanceId = '', candidateId = '', role = '' } = {}) {
  return `target-${stableReviewId(['target', labelKind, entityId, visualInstanceId, candidateId, role].join('\0'))}`;
}

export function labelIdFor({ runKey = '', captureKey = '', passId = 'default', reviewerId = 'unassigned', targetKey = '' } = {}) {
  return `label-${stableReviewId(['label', REVIEW_VERSION, runKey, captureKey, passId, reviewerId, targetKey].join('\0'))}`;
}

export function draftStorageKey({ runKey = '', captureKey = '', passId = 'default', reviewerId = 'unassigned' } = {}) {
  return `logo-yoink.visual-review.v2.${stableReviewId([REVIEW_VERSION, runKey, captureKey, passId, reviewerId].join('\0'))}`;
}
...
export async function loadBundle(runDirectory) {
  const manifest = await readJson(join(runDirectory, 'manifest.json'), null) ?? await readJson(join(runDirectory, 'benchmark-manifest.json'), {});
...
async function renderEntity(entity, assetResolver, outputDirectory, packetHref, reviewContext) {
  ...
  const instanceCards = await Promise.all(entity.visual_instances.map(async instance => {
    ...
    return `<article class="instance-card" data-instance-id="${escapeHtml(instance.visual_instance_id)}"><div class="instance-images"><figure><figcaption>Rendered instance</figcaption>${mediaFigure({ path: instance.screenshot_path }, instanceScreenshotHref, `${entity.name} rendered instance`)}</figure><figure><figcaption>Element crop</figcaption>${mediaFigure({ path: instance.crop_path }, cropHref, `${entity.name} element crop`)}</figure><div><b>Mapping</b><p>${escapeHtml(instance.candidate_id || 'unmapped')} · ${escapeHtml(instance.mapping_type || 'not classified')}</p>${mapped ? `<div class="mapped-preview">${await candidatePreview(mapped, assetResolver, outputDirectory, 'mapped candidate')}</div>` : '<div class="placeholder compact">No mapped candidate</div>'}</div></div>${snapshotDetails(instance.feature_snapshot, instance.evidence_snapshot)}<form class="instance-form" data-record-type="visual-instance" data-entity-id="${escapeHtml(entity.entity_id)}" data-visual-instance-id="${escapeHtml(instance.visual_instance_id)}" data-candidate-id="${escapeHtml(instance.candidate_id)}">${selectControl('identity', ['correct', 'wrong', 'ambiguous'], 'Identity')}${selectControl('visual-role', ['symbol', 'wordmark', 'horizontal-lockup', 'stacked-lockup', 'favicon', 'social-card', 'badge', 'partner-logo', 'UI-control', 'content-image', 'other'], 'Visual role')}${selectControl('region', ['header', 'nav', 'body', 'footer', 'metadata', 'browser-chrome', 'other'], 'Region')}${selectControl('theme', ['light', 'dark', 'both', 'unknown'], 'Theme')}${selectControl('visibility', ['good', 'conditional', 'unusable'], 'Visibility')}${selectControl('first-party', ['yes', 'no', 'ambiguous'], 'First party')}${selectControl('mapping-confidence', ['exact', 'derived', 'suggested', 'unmapped'], 'Mapping confidence')}${textControl('confidence', 'Confidence')}${textControl('note', 'Note', true)}</form></article>`;
  }));

  const missingControls = ROLES.map(role => `<form class="missing-form" data-record-type="missing-role" data-entity-id="${escapeHtml(entity.entity_id)}" data-role="${escapeHtml(role)}"><b>${escapeHtml(role)}</b>${selectControl('missing-cause', ['no_graphic_asset_exists', 'icon_or_stacked_only', 'asset_visible_not_discovered', 'discovered_not_validated', 'removed_by_dedupe', 'excluded_by_budget', 'rejected_identity_or_evidence', 'rejected_shape_or_quality', 'ranked_below_worse_candidate', 'theme_serialization_failure', 'page_blocked_or_incomplete', 'identity_unsafe_or_ambiguous', 'not_missing'], 'Missing cause')}${textControl('confidence', 'Confidence')}${textControl('note', 'Note', true)}</form>`).join('');
...
(() => {
  ...
  const scopeToken = () => stableId([root?.dataset.reviewVersion || '${REVIEW_VERSION}', root?.dataset.runId || 'run', root?.dataset.captureKey || 'capture', reviewer('pass'), reviewer('id')].join('\0'));
  const storageKey = () => 'logo-yoink.visual-review.v2.' + scopeToken();
  function stableId(value) { let hash = 2166136261; for (const character of String(value)) { hash ^= character.charCodeAt(0); hash = Math.imul(hash, 16777619); } return (hash >>> 0).toString(16).padStart(8, '0'); }
  function record(form) {
    const values = Object.fromEntries([...form.querySelectorAll('[data-field]')].map(control => [control.dataset.field, control.value || '']).filter(([, value]) => value !== ''));
    if (!Object.keys(values).length) return null;
    const type = form.dataset.recordType;
    const labelKind = type === 'visual-instance' ? 'visual_instance' : type === 'missing-role' ? 'missing_role' : type;
    const entityId = form.dataset.entityId || '';
    const visualInstanceId = form.dataset.visualInstanceId || null;
    const candidateId = form.dataset.candidateId || null;
    const role = form.dataset.role || values['visual-role'] || values['best-role'] || null;
    const targetKey = 'target-' + stableId(['target', labelKind, entityId, visualInstanceId || '', candidateId || '', role || ''].join('\0'));
    const labelId = 'label-' + stableId(['label', root?.dataset.reviewVersion || '${REVIEW_VERSION}', root?.dataset.runId || 'run', root?.dataset.captureKey || 'capture', reviewer('pass'), reviewer('id'), targetKey].join('\0'));
    return { schema_version: 'visual-benchmark-v1', record_type: 'label', label_id: labelId, target_key: targetKey, label_kind: labelKind, entity_id: entityId, candidate_id: candidateId, visual_instance_id: visualInstanceId, role, values, reviewer_id: reviewer('id'), reviewer_kind: reviewer('kind'), review_pass: reviewer('pass'), run_key: root?.dataset.runId || 'run', capture_key: root?.dataset.captureKey || 'capture', reviewed_at: new Date().toISOString(), provenance: { schema_version: 'visual-benchmark-v1', capture_version: root?.dataset.captureKey || '${REVIEW_VERSION}', task_id: null, model: null, prompt_version: '${REVIEW_VERSION}' } };
  }
  function records() { return [...root.querySelectorAll('form[data-record-type]')].map(record).filter(Boolean); }
  function save() { const lines = records().map(item => JSON.stringify(item)); try { localStorage.setItem(storageKey(), lines.join('\\n') + (lines.length ? '\\n' : '')); status.textContent = 'Draft saved as JSONL; blank fields remain unlabeled.'; } catch { status.textContent = 'Draft available for export; browser storage is unavailable.'; } }
  function download() { const lines = records().map(item => JSON.stringify(item)); const blob = new Blob([lines.join('\\n') + (lines.length ? '\\n' : '')], {type:'application/x-ndjson'}); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = 'visual-review-' + (root.dataset.runId || 'labels') + '.jsonl'; link.click(); setTimeout(() => URL.revokeObjectURL(url), 1000); status.textContent = 'JSONL exported; review fields are empty until selected.'; }
  function restore() { try { const text = localStorage.getItem(storageKey()); if (!text || text.length > ${MAX_INPUT_FILE_BYTES}) return; const saved = text.split('\\n').filter(Boolean).slice(0, ${MAX_JSONL_ROWS}).map(JSON.parse); for (const item of saved) { const formType = item.label_kind === 'visual_instance' ? 'visual-instance' : item.label_kind === 'missing_role' ? 'missing-role' : item.label_kind; const selector = 'form[data-record-type="' + CSS.escape(formType || '') + '"][data-entity-id="' + CSS.escape(item.entity_id || '') + '"]'; const forms = [...root.querySelectorAll(selector)].filter(form => (!item.visual_instance_id || form.dataset.visualInstanceId === item.visual_instance_id) && (!item.candidate_id || form.dataset.candidateId === item.candidate_id) && (!item.role || !form.dataset.role || form.dataset.role === item.role)); const form = forms[0]; if (!form) continue; for (const control of form.querySelectorAll('[data-field]')) control.value = item.values?.[control.dataset.field] || ''; } status.textContent = 'Saved JSONL draft restored; empty fields remain unlabeled.'; } catch { status.textContent = 'Draft could not be restored; review fields are empty.'; } }
  root.addEventListener('input', save); root.addEventListener('change', save); document.querySelector('[data-action=save]')?.addEventListener('click', save); document.querySelector('[data-action=export]')?.addEventListener('click', download);
  restore();
})();
</script>`;
}
...
export async function buildReviewPacket({ runDirectory, outputDirectory, resume = false, entityId = null, reviewerId = '', reviewerKind = '', passId = 'default' }) {
  ...
  const packets = entityId ? allPackets.filter(packet => packet.entity_id === String(entityId)) : allPackets;
  ...
  for (const entity of packets) {
    ...
    const html = await renderEntity(entity, assetResolver, entitiesDirectory, '../index.html', { runKey, captureKey, reviewerId, reviewerKind, passId });
...
async function main() {
  try {
    ...
    const result = await buildReviewPacket({ runDirectory: options.run, outputDirectory: options.output, entityId: options.entity, resume: options.resume, reviewerId: options.reviewer, reviewerKind: options.reviewerKind, passId: options.pass ?? 'default' });
