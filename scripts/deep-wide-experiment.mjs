...
if (!controlPath) throw new Error('Usage: node scripts/deep-wide-experiment.mjs <control results.jsonl> [output] [limit]');
...
function selectedCandidate(record, role) {
  const id = record.selected_by_role?.[role];
  return record.candidates?.find(candidate => candidate.candidate_id === id) ?? null;
}
