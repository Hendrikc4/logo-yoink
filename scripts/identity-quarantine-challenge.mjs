...
function privateIp(hostname) {
  ...
  if (hostname === '::' || hostname === '::1' || /^(?:fc|fd|fe[89ab])/i.test(hostname)) return true;
  const parts = hostname.split('.').map(Number);
  return parts[0] === 0 || parts[0] === 10 || parts[0] === 127 ||
    (parts[0] === 100 && parts[1] >= 64 && parts[1] <= 127) ||
    (parts[0] === 169 && parts[1] === 254) ||
    (parts[0] === 172 && parts[1] >= 16 && parts[1] <= 31) ||
    (parts[0] === 192 && (parts[1] === 168 || parts[1] === 0)) || parts[0] >= 224;
}
...
async function assertPublic(url) {
  ...
  if (host === 'localhost' || host.endsWith('.local') || privateIp(host)) throw new Error('non-public URL');
  if (!isIP(host)) {
    const addresses = await dnsLookup(host, { all: true, verbatim: true });
    if (!addresses.length || addresses.some(item => privateIp(item.address))) throw new Error('non-public DNS result');
  }
}
