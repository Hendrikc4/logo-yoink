...
function draw() {
  ...
  for (const entity of state.entities) {
    if (entity.type === 'cactus') context.drawImage(assets.cacti[entity.artVariant ?? 0], entity.x - 10, entity.y, entity.width + 20, entity.height);
  ...
  if (!invulnerableBlink) {
    ...
    const riderX = ['running', 'paused'].includes(state.status) ? RIDER_DRAW_X : canvas.clientWidth < 650 ? 372 : 732;
    ...
    context.drawImage(riderImage, riderX, state.rider.y - RIDER_DRAW_Y_OFFSET + bob, RIDER_DRAW_SIZE, RIDER_DRAW_SIZE);
  }
