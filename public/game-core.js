...
export function createGame({ seed = 0x59f15e, logoCount = 12 } = {}) {
  ...
  return {
    ...
    obstacleInMs: 1750,
    logoInMs: 900,
    lassoInMs: between({ rngState: normalizedSeed }, 12000, 18000),
    collected: 0,
    events: [],
  };
}
...
function collectLogo(state, entity, automatic = false) {
  state.logoPoints = (state.logoPoints ?? 0) + 100;
  state.collected += 1;
...
function collectLasso(state, entity) {
  ...
  entity.remove = true;
  state.lassoInMs = between(state, 25000, 40000);
}
...
function spawnObstacle(state) {
  ...
  addEntity(state, { type: 'cactus', x: WORLD.width + 70, y: WORLD.groundY - size.height, ...size });
  const safeGapPx = state.speed * 1.32 + 170;
  state.obstacleInMs = safeGapPx / state.speed * 1000 + between(state, 280, 760);
}
...
export function setQaState(state, qaState) {
  ...
  if (qaState === 'lasso') {
    ...
    addEntity(state, { type: 'logo', x: 760, y: WORLD.groundY - 215, logoIndex: 7 });
    state.status = 'paused';
    ...
    state.logoInMs = 999999;
    state.lassoInMs = 999999;
    CACTUS_SIZES.forEach((size, index) => addEntity(state, {
      type: 'cactus', x: 650 + index * 230, y: WORLD.groundY - size.height, ...size,
    }));
    state.status = 'paused';
