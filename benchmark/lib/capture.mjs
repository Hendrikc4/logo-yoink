...
import { extractLogos as defaultExtractLogos } from '../../src/extractor.mjs';
import { canonicalHostname, isPrivateIp } from '../../src/network-safety.mjs';
...
function stableVisualKey(entityId, view, instance) {
  ...
  // View/theme is intentionally not part of the key by itself: identical
  // observations from a top screenshot, tile, or equivalent responsive view
  // are one instance. Material visual changes (content, geometry, or rendered
  // colors) naturally alter the key and remain separate.
  return `${entityId}\0${instance?.kind ?? 'other'}\0${instance?.url ?? `svg:${instance?.inline_svg_hash ?? ''}`}\0${instance?.region ?? 'unknown'}\0${Math.round(instance?.box?.x ?? 0)}\0${Math.round(instance?.box?.y ?? 0)}\0${Math.round(instance?.box?.width ?? 0)}\0${Math.round(instance?.box?.height ?? 0)}\0${locator.id ?? ''}\0${locator.class_name ?? ''}\0${evidence.computed_color ?? ''}\0${evidence.background_color ?? ''}`;
}

function addTruncationReason(diagnostics, reason) {
  if (!diagnostics.truncation_reasons.includes(reason)) diagnostics.truncation_reasons.push(reason);
}

function addCropTruncationReason(diagnostics, reason) {
  diagnostics.cropEvidenceTruncated = true;
  if (!Array.isArray(diagnostics.cropTruncationReasons)) diagnostics.cropTruncationReasons = [];
  if (!diagnostics.cropTruncationReasons.includes(reason)) diagnostics.cropTruncationReasons.push(reason);
  if (!diagnostics.warnings.includes('crop-evidence-truncated')) diagnostics.warnings.push('crop-evidence-truncated');
}

function instanceBudgetKey(view, instance) {
  return stableVisualKey('', view, instance);
}
...
async function buildEvidenceRows(entity, target, finalUrl, staticResult, views, config, diagnostics) {
  ...
  for (const view of views) for (const instance of view.instances ?? []) {
    const visualKey = stableVisualKey('', view, instance);
...
async function captureView(page, view, entityDirectory, options, diagnostics, instanceKeys) {
  ...
  const inspectWithBudget = async prefix => {
    // Inspect up to the hard per-observation bound, then charge only stable
    // unique keys. A lazy-loaded element is commonly observed once in the top
    // screenshot and again in one or more scroll tiles; those repeats must not
    // consume the global instance budget.
    const found = await inspectInstances(page, view, { ...options, maxInstances: HARD_LIMITS.instances }, prefix);
    const unique = [];
    for (const instance of found) {
      const key = instanceBudgetKey(view, instance);
      if (instanceKeys.has(key)) continue;
      if (instanceKeys.size >= options.maxInstances) {
        diagnostics.instanceBudgetOmitted = true;
        diagnostics.budgetTruncated = true;
        addTruncationReason(diagnostics, 'instance-budget');
        continue;
      }
      instanceKeys.add(key);
      unique.push(instance);
    }
    diagnostics.instanceCount = instanceKeys.size;
    return unique;
  };
  ...
  for (const [index, instance] of instances.entries()) {
    ...
    cropEligible.push({ index, instance, box, scrollY, clip });
  }
  const availableCrops = Math.max(0, options.maxCrops - diagnostics.cropCount);
  if (cropEligible.length > availableCrops) {
    addCropTruncationReason(diagnostics, 'crop-budget');
  }
  for (const { index, instance, scrollY, clip } of cropEligible.slice(0, availableCrops)) {
    await page.evaluate?.(scrollToY, scrollY).catch?.(() => {});
    await page.waitForTimeout?.(25);
    const crop = await screenshot(page, join(entityDirectory, 'element-crops', artifactName(view.id, 'crop', index + 1)), options, diagnostics, { clip });
...
export async function captureEntity(input, options = {}) {
  ...
  const diagnostics = { requests: 0, declared_bytes: 0, blocked_requests: 0, responses: [], screenshots: 0, screenshotBytes: 0, instanceCount: 0, cropCount: 0, cropEvidenceTruncated: false, cropTruncationReasons: [], budgetTruncated: Boolean(config.budgetTruncated), truncation_reasons: [], resourceLimitHit: false, warnings: [], artifactFailures: [], errors: [] };
  ...
  try {
    ...
    const views = [];
    for (const view of config.views) views.push(await captureView(page, view, entityDirectory, config, diagnostics, instanceKeys));
    ...
    if (budget.limitHit) { diagnostics.budgetTruncated = true; addTruncationReason(diagnostics, 'network-budget'); }
...
export function inspectPage({ view, maxInstances = 120, maxInlineSvgs = 24, maxInlineSvgBytes = 256 * 1024 } = {}) {
  ...
  const http = value => { try { const raw = clean(value); if (!raw || /^(?:null|undefined|about:blank|blob:)/i.test(raw) || /(?:^|\/)null(?:[?#].*)?$/i.test(raw)) return null; const url = new URL(raw, document.baseURI); return /^https?:$/.test(url.protocol) ? url.href : null; } catch { return null; } };
