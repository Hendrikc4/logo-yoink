...
- Score equals continuously increasing distance points plus 100 points per collected logo. Best score persists locally when storage is available.
...
- While armed, each approaching logo receives a visible rope arc, is pulled toward the rider without requiring a jump, awards the normal 100 points, decrements the counter, and triggers a large `YOINK!` animation. The lasso does not protect against cacti.
